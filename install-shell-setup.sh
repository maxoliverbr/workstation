...
# Require at least 5 GB free on / (dnf, flatpak, and most installs use root)
...
# DNF wrapper: verbose by default, quiet with --silent
dnf_quiet() { if [ "$SILENT" = "1" ]; then sudo dnf "$@" >&3 2>&3; else sudo dnf "$@"; fi; }
...
# Install a group of dnf packages in one call, skipping those already present
